...
my ($path, $THRES_siteDepCut, $THRES_minorAlleDepCut, $THRES_sampSiteCut,
		...
		$FILE_excludeRegion, $INT_excludeDist, $OUTPUT_SITE_ALL_SAMPLE_HAS_SNP) = load_arg(\@ARGV);
...
printf(OUT "%-15s\t%-10s\t%-10s\t%-10s\t%-10s\t%-10s\t", "sampleId","validPosNo","#iSNV(final)","#SNP","meanSiteDep","IsSampleValid?");
print "#iSNV_in_excluding_region" if ($FILE_excludeRegion ne 'NULL');
print OUT "#iSNV_in_excluding_region" if ($FILE_excludeRegion ne 'NULL');
print "\n";
print OUT "\n";
...
for (my $i = 0; $i < @samples; $i ++){
	...
	foreach my $p (@positions){
		...
		($rh_siteMinorDep->{$samples[$i]}->{$p} ne 'na' && $rh_siteMinorDep->{$samples[$i]}->{$p} >= $THRES_minorAlleDepCut) ) {
			...
			if ($flag_iSNVpassStrandedFilter == 1 && $rh_siteMinorFreq->{$samples[$i]}->{$p} >= $THRES_iSNVCut){
				...
				if ($patt[0] ne $patt[2] && 
				    $rh_siteMaxStranded->{$samples[$i]}->{$p} >= $THRES_strandedRatioCut && $rh_siteMaxStranded->{$samples[$i]}->{$p} <= 1-$THRES_strandedRatioCut &&! defined $rh_pos2isprimer->{$p}){
						$rh_SNP->{$samples[$i]}->{$p} = int($rh_siteMinorFreq->{$samples[$i]}->{$p}*10000)/10000;
						$cValid_SNP ++;
						if (defined $rh_posHasSNP->{$p}){
							$rh_posHasSNP->{$p} ++;
						} else {
							$rh_posHasSNP->{$p} = 1;
						}
	...
	printf(OUT "%-15s\t%-10d\t%-10d\t%-10d\t%-10d\t",$samples[$i], $cValidPos, $cValid_iSNV, $cValid_SNP, $siteMeanDep);
	...
	print OUT "$ciSNVtoExcludeInPrimer" if ($FILE_excludeRegion ne "NULL");
	print "\n";
	print OUT "\n";
}
...
print OUT "\nTotally, $cSampValid samples are valid.\n";
print OUT "\tvalid_site: dep >= $THRES_siteDepCut (ignoring indels) or minor_dep>=$THRES_minorAlleDepCut\n";
print OUT "\t#valid_site >= $THRES_sampSiteCut\n";
print OUT "iSNV identification cutoff:\n";
print OUT "\tminor freq >= $THRES_iSNVCut\n";
print OUT "\tsecond allele positive stranded belongs to [$THRES_strandedRatioCut to ",1-$THRES_strandedRatioCut,"]\n";
print OUT "SNP identification cutoff:\n";
print OUT "\tminor freq < $THRES_iSNVCut\n";
print OUT "\tmajor allele positive stranded belongs to [$THRES_strandedRatioCut to ",1-$THRES_strandedRatioCut,"]\n";
...
print OUT "#NOTE: (1) mutated allele frequencies are output, instead of minor allele frequencies. (2) NO = no isnv or freq < cutoff, denoted as -1.0 in iSNV.all.formatlab; (3) NA = this pos is invalid for iSNV calling, denoted as -2.0 in iSNV.all.formatlab\n\n";
print OUT "pos\tiSNV#\t";
...
print "\n#output summary of both iSNVs and SNP to iSNV_with_SNP.all.txt ...\n";
open OUT, "> iSNV_with_SNP.all.txt" or die "$!: iSNV_with_SNP.all.txt\n";
print OUT "#NOTE: (1) mutated allele frequencies are output; (2) NO = no isnv or freq < cutoff or no SNP; (3) NA = this pos is invalid for iSNV or SNP calling. Sites with SNPs (maybe not the same allele) at all samples are excluded in this output.\n\n";
print OUT "pos\tiSNV/SNP#\t";
my @SNPPos = keys %$rh_posHasSNP;
my $rh_pos;
foreach (@SNPPos) { $rh_pos->{$_} = 1;}
foreach (@iSNVPos) { $rh_pos->{$_} = 1;}
my @bothPos = sort {$a <=> $b} keys %$rh_pos;
...
for (my $j = 0; $j < @bothPos; $j ++){
	my $p = $bothPos[$j];
	if ($OUTPUT_SITE_ALL_SAMPLE_HAS_SNP == 0){
		next if (defined $rh_posHasSNP->{$p} && $rh_posHasSNP->{$p} == @samples);
	}
	print OUT "$p\t";
	if (defined $rh_posHasiSNV->{$p}){
		print OUT "$rh_posHasiSNV->{$p}/";
	} else {
		print OUT "-/";
	}	
	if (defined $rh_posHasSNP->{$p}){
		print OUT "$rh_posHasSNP->{$p}\t";
	} else {
		print OUT "-\t";
	}	
	...
	for (my $i = 0; $i < @samples; $i ++){
		if (defined $rh_iSNV->{$samples[$i]}->{$p} ){
			...
			if ($patt[0] eq $patt[1]){
				...
				print OUT 1-$rh_iSNV->{$samples[$i]}->{$p},"\t";
			}
		} elsif (defined $rh_SNP->{$samples[$i]}->{$p} ) {
			print OUT 1-$rh_SNP->{$samples[$i]}->{$p},"\t";
		} elsif (defined $rh_validPos->{$samples[$i]}->{$p}) {
			print OUT "NO\t";
		} else {
			print OUT "NA\t";
...
sub load_arg {
	...
	for(my $i = 0; $i < @args; $i+=2){
		...
		} elsif ($args[$i] eq '-output_sites_all_with_snp'){
			$out_fullSNPsite = $args[$i+1];
			if ($out_fullSNPsite ne '1' && $out_fullSNPsite ne '0'){
				help_info();
				die "[error] -output_sites_all_with_snp $out_fullSNPsite: -output_sites_all_with_snp requires <0|1>\n";
			}
		} else {
			help_info();
	...
	return ($input, $d, $c, $s, $f, $st_t, $st_c, $pf, $pf_d, $out_fullSNPsite);
}
...
sub help_info{
	...
	print "\t-output_sites_all_with_snp <0|1>\n";
	print "\n";
}
